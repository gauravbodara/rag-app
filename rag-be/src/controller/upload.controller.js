...
const { createVectorStoreFromDocs } = require('../service/embedding.service.js');
...
const opentracing = require('opentracing');
 const handleUpload = async (req, res) => {
  ...
  try {
    ...
    chunkingSpan.log({ event: 'chunking_time', value: chunkingEnd - chunkingStart });
    chunkingSpan.finish();
    const embeddingSpan = tracer.startSpan('embedding', { childOf: parentSpan });
    ...
    embeddingSpan.log({ event: 'embedding_time', value: embeddingEnd - embeddingStart });
